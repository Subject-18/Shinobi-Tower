...
{   
    ...
    [SerializeField] float ledgeRayLength;

    [SerializeField] float ledgeHeightThreshold = 0.75f;

    [SerializeField] float climbLedgelength = 1.5f;
    ...
    public ObstacleData ObstacleCheck()
    {
        ...
        if(hitData.forwardHitFound){
            ...
            Physics.Raycast(heightOrigin, Vector3.down, out hitData.heightHit, 
    ...
    public bool climbLedgeCheck(Vector3 dir, out RaycastHit ledgeHit)
    {
        ...
        for (int i = 0; i < 12; i++)
        {
            ...
            if(Physics.Raycast(origin+offset*i, dir, out RaycastHit hit,climbLedgelength, 
            climbLedgeLayer))
            {
                ledgeHit = hit;
                return true;
    ...
    // FOr checking the ledge below the drop point 
    public bool DropLedgeCheck( out RaycastHit DropHit)
    {
        DropHit = new RaycastHit();

        var origin = transform.position + Vector3.down * 0.1f + transform.forward * 0.5f;
    ...
    public bool ledgeCheck(Vector3 moveDir, out LedgeData ledgeData){
        ...
        var origin = transform.position + moveDir * originOffset + Vector3.up;
         ...
         if(RayCast.threerayCast(origin, Vector3.down, 0.3f, transform,  out List<RaycastHit>hits, ledgeRayLength, obstacleLayer))
        {
            ...
            if(validHits.Count > 0)
                {
                    ...
                    if (Physics.Raycast(surfaceRayOrigin, transform.position - surfaceRayOrigin, 
